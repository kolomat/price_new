...
class ControllerExtensionModuleZoXml2 extends Controller {
    ...
    $data['breadcrumbs'][] = array(
		...
		$data['real_token']  = $this->session->data['token'];
// -----------------------------------------------------------------------------
...
if (isset($this->request->get['filter_supplier'])) {
  ...
  $data['token'] .= "&filter_supplier=" . $data['filter_supplier'];
  ...
  if ($data['module']['default_supplier']!='all') {
    ...
    $data['token'] .= "&filter_supplier=" . $data['module']['default_supplier'];
...
foreach ($query->rows as $result) {
  ...
  if (isset($value->ms_seller))             $data['suppliers'][$result['session_key']]['settings']['ms_seller'] = $value->ms_seller;
  ...
  if (isset($value->user_exceptions_attributes)) $data['suppliers'][$result['session_key']]['settings']['user_exceptions_attributes'] = (array)$value->user_exceptions_attributes;
...
$types                 = array('select','radio','checkbox','image','checkbox_qty','radio_qty','image_qty','input_qty','input_qty_ns','input_qty_td','select_qty','area_hw','list','oct_quantity');
...
//      if (count($data['suppliers'][$result['session_key']]['log']) > 50) break;
      }
    }
  }
...
// -----------------------------------------------------------------------------
  $query = $this->db->query("SHOW TABLES LIKE '" . DB_PREFIX . "ms_seller'");
  if ($query->row) {
    $data['ms_sellers'] = array();
    $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "ms_seller");
    foreach ($query->rows as $result) $data['ms_sellers'][$result['seller_id']] = $result['nickname'];
    }
// -----------------------------------------------------------------------------
	$data['header'] = $this->load->controller('common/header');
	$data['column_left'] = $this->load->controller('common/column_left');
	$data['footer'] = $this->load->controller('common/footer');
  $this->response->setOutput($this->load->view('extension/module/zoxml2.tpl', $data));
	}
  ...
  public function loadcategorypage() {
    $json = array ();
      ...
      if (isset($this->request->post['supplier'])&&isset($this->request->post['page'])) {
        ...
        if (isset($this->request->post['limit']))               $conditions .= " LIMIT " . (int)$this->request->post['page']*$module['categoty_step'] . ", " . (int)$module['categoty_step'];                                    
...
//$this->db->query("INSERT INTO " . DB_PREFIX . "zoxml2_log SET session_key = '0', type = 'debug', data = '" . $this->db->escape("addfeed") . "', user = ''");
    ...
    foreach ($mquery->rows as $result) $module[$result['key']] = json_decode ($result['data']);
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
//$this->db->query("INSERT INTO " . DB_PREFIX . "zoxml2_log SET session_key = '0', type = 'debug', data = '" . $this->db->escape("POST") . "', user = ''");
      if (isset($this->request->post['add'])) {
//$this->db->query("INSERT INTO " . DB_PREFIX . "zoxml2_log SET session_key = '0', type = 'debug', data = '" . $this->db->escape("add") . "', user = ''");
        $action = array();
...
//          $this->db->query("INSERT INTO " . DB_PREFIX . "zoxml2_log SET session_key = '0', type = 'debug', data = '" . $this->db->escape("http_response_status - " . $http_response_status) . "', user = '" . $this->db->escape($action['user']) . "'");
  ...
  public function install() {
    ...
    $data = "module=Диспетчер XML v. 2.3";
    $ch = curl_init(); 
    $data.= "&mail=";
  ...
  public function addallvendors () {
      ...
      if (isset($this->request->post['supplier'])) {
        ...
        foreach ($manufacturer_info->rows as $result) {
          ...
          // ЛЕЧИМ КРИВЫХ
          if ($manufacturer_description) {
            $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "manufacturer_description WHERE manufacturer_id = '" . (int)$result['manufacturer_id'] . "'");
            if (!$query->row) {
              $extra = '';
              if ($field_name)        $extra .= "name       = '" . $this->db->escape($result['name'])      . "', "; 
              if ($field_meta_title)  $extra .= "meta_title = '" . $this->db->escape($result['name'])      . "', "; 
              if ($field_meta_h1)     $extra .= "meta_h1    = '" . $this->db->escape($result['name'])      . "', "; 
		          $this->db->query("INSERT INTO " . DB_PREFIX . "manufacturer_description SET  " . $extra . "  manufacturer_id = '" . (int)$result['manufacturer_id'] . "', language_id = '" . (int)$this->config->get( 'config_language_id' ) . "'");
              }
            }
          }
            ...
            if ($manufacturer_id===FALSE) {
              ...
              if ($do_vendor_seourl) {
                $vendor_seourl = $this->translit ($name);
			          $this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = 'manufacturer_id=" . (int)$manufacturer_id . "', keyword = '" . $this->db->escape($vendor_seourl) . "'");
                }
              $this->db->query("UPDATE " . DB_PREFIX . "zoxml2_vendors SET `manufacturer_id` = '" . (int)$manufacturer_id . "' WHERE `id` = '" . (int)$result_id . "'");
              }
            else {
              $this->db->query("UPDATE " . DB_PREFIX . "zoxml2_vendors SET `manufacturer_id` = '" . (int)$manufacturer_id . "' WHERE `id` = '" . (int)$result_id . "'");
              }
...
public function docategoryseourl () {
  if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
      ...
      $entry = 'category_id=' . (int)$result['category_id'];
		  $url_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "url_alias WHERE query = '" . $this->db->escape($entry) . "'");
      if (empty($url_query->row['keyword'])) {
		    $this->db->query("DELETE FROM " . DB_PREFIX . "url_alias WHERE query = '" . $this->db->escape($entry) . "'");
		    $this->db->query("INSERT INTO " . DB_PREFIX . "url_alias SET query = '" . $this->db->escape($entry) . "', keyword = '" . $this->db->escape($this->translit ($result['name'])) . "'");
        }
      }
    }  
