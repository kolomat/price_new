 ...
 */
//Версия модуля
define ('YANDEX_YML_VERSION', '1.8.7');
...
//---- Config section ----
	...
	*/
	protected function getYmlHeader() {
		$this->load->model('extension/feed/yandex_yml');
		...
		$this->categories = $this->model_extension_feed_yandex_yml->getCategoryTree($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_ids, $allowed_manufacturers, $this->is_main_category);
	...
	protected function getYmlChunk($page = 0) {
		...
		if ($this->numpictures > 1) {
			...
			$product_images = $this->model_extension_feed_yandex_yml->getProductImages($this->numpictures - 1);
			//---- Дополнительные изображения товара ----
		}
		$all_attributes = $this->model_extension_feed_yandex_yml->getAttributes($this->config->get($this->CONFIG_PREFIX.'attributes'));
		...
		foreach ($this->model_extension_feed_yandex_yml->getProduct($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_ids, $allowed_manufacturers, $customer_group, $product_rel || $product_accessory, $skip, $limit, $this->is_main_category) as $product) {
			$is_out = false;
			$data = array();
...
/*			if ($this->numpictures > 0) {
			...
			*/
			...
			$data['param'] = array();
			$attributes = $this->model_extension_feed_yandex_yml->getProductAttributes($product['product_id']);
			$attr_text = array();
	...
	*/
	...
	protected function setOptionedOffer($data, $product, $shop_currency, $offers_currency, $decimal_place) {
		...
		if ($this->color_options)
			$coptions = $this->model_extension_feed_yandex_yml->getProductOptions($this->color_options, $product['product_id']);
		$soptions = array();
		if ($this->size_options)
			$soptions = $this->model_extension_feed_yandex_yml->getProductOptions($this->size_options, $product['product_id']);
	...
	*/
	...
	protected function setOffer($data) {
		...
		$allowed_tags = array('url'=>0, 'buyurl'=>0, 'price'=>1, 'oldprice'=>0, 'wprice'=>0, 'currencyId'=>1, 'xCategory'=>0, 'categoryId'=>1, 'partType'=>0, 'partCount'=>0, 'picture'=>0, 'store'=>0, 'pickup'=>0, 'delivery'=>0, 'deliveryIncluded'=>0, 'local_delivery_cost'=>0, 'orderingTime'=>0, 'min-quantity'=>0, 'step-quantity'=>0, 'stock_quantity'=>0);
		...
		$allowed_tags = array_merge($allowed_tags, array('aliases'=>0, 'additional'=>0, 'description'=>0, 'name_ua'=>0, 'description_ua'=>0, 'sales_notes'=>0, 'promo'=>0, 'manufacturer_warranty'=>0, 'country_of_origin'=>0, 'weight'=>0, 'downloadable'=>0, 'adult'=>0, /*'age'=>0,*/ 'barcode'=>0, 'keywords'=>0, 'rec'=>0));
