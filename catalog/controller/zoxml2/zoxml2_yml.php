      ...
      while ($prohodov--) {
        ...
        foreach ($input_categories as $item) {
          ...
          if ($item_id) {
            ...
            // ЗАЩИТА ОТ ИДИОТОВ КОТОРЫЕ ДЕЛАЮТ ЗАЦИКДЕННЫЕ КАТЕГОРИИ <category id="1" parentId="1">блуза</category>
            if ($parent_id == $item_id) $parent_id    = 0;
...
//  $data['info_progress']    = 0;
  ...
  if (!empty($data['settings']['user_start'])) {
    ...
    if (isset($data['offers'])) $offers = $data['offers'];
    else {
      if ($data['settings']['tag_shop']) {
        if ($data['settings']['tag_offers']) $offers = $xml->{$data['settings']['tag_shop']}->{$data['settings']['tag_offers']}->{$data['settings']['tag_offer']};
        else                                 $offers = $xml->{$data['settings']['tag_shop']}->{$data['settings']['tag_offer']};
    ...
    $this->db->query("INSERT INTO " . DB_PREFIX . "zoxml2_log SET session_key = '" . $this->db->escape($session_key) . "', type = 'info', data = '" . $this->db->escape("Найдено товаров: " . count($offers) . ". Старт обработки") . "', user = '" . $this->db->escape($data['user']) . "'");
    foreach ($offers as $item) {
      $info_processed ++;
      if (++$info_progress==$data['module']['step']) {
        $info_progress = 0; 
...
//           		$this->db->query("DELETE FROM " . DB_PREFIX . "product_input_price  WHERE product_id = '" . (int)$product_id . "'");
    ...
    if ((isset($data['settings']['hide_missing']) || isset($data['settings']['zero_missing'])) && isset($data['process_start_time'])) {
      ...
      $this->db->query("INSERT INTO " . DB_PREFIX . "zoxml2_log SET session_key = '" . $this->db->escape($session_key) . "', type = 'info', data = '" . $this->db->escape('Было найдено пропущенных товаров: ' . $countAffected) ."', user = '" . $this->db->escape($data['user']) . "'");
      }
