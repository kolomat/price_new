...
if ($script_name=='restruct_pl') {
        $categories = array();
        $match_cats;
        $this->db->query("INSERT INTO " . DB_PREFIX . "zoxml2_log SET session_key = '" . $this->db->escape($session_key) . "', type = 'DEBUG', data = '" . $this->db->escape('Start restruct_pl!') . "', user = '" . $this->db->escape($user) . "'");
        $src = str_replace('<articles>', '<shop><offers>', $src);
        $src = str_replace('</articles>', '</offers></shop>', $src);
        $src = preg_replace_callback('/<category_keys>([\s\S]+?)<\/category_keys>?/mi', function($match_cat) use (&$last_id, &$categories){
            $last_id = null;
            foreach(preg_split('/\|/', $match_cat[1]) as $category)
            {
                $parent_id = $last_id;
                $last_id .= $category;
                $md_id = md5($last_id);
                if(!empty($categories[$md_id])) continue;
                $categories[$md_id] = ['id' => $md_id, 'parent_id' => (!empty($parent_id) ? md5($parent_id) : null), 'name' => $category];
            }
            return "<categoryId>" . $md_id . "</categoryId>";
        }, $src);
        $this->db->query("INSERT INTO " . DB_PREFIX . "zoxml2_log SET session_key = '" . $this->db->escape($session_key) . "', type = 'DEBUG', data = '" . $this->db->escape('Create offers from units...') . "', user = '" . $this->db->escape($user) . "'");
        $src = preg_replace_callback('/<article[\s\S]+?>([\s\S]+?)<units>([\s\S]+?)<\/units>?[\s]*?<\/article>?/mi', function($matches){
            $matches[1] = preg_replace('/[\s]*<\/{0,1}attributes>?/mi', '', $matches[1]);
            $matches[1] = preg_replace('/[\s]*<\/{0,1}image[s]{0,1}>/mi', '', $matches[1]);
            $matches[1] = preg_replace_callback('/<brand>\s*?<name>([\s\S]+?)<\/name>\s*?<\/brand>?/mi', function($matches2) use ($matches){
                return "<brand>" . $matches2[1] . "</brand>";
            }, $matches[1]);
            $matches[2] = preg_replace_callback('/(<unit[\s\S]+?>)([\s\S]+?)(<\/unit>?)/mi', function($matches2) use ($matches){
                $matches2[2] = preg_replace_callback('/<image_url>[^<]*?<\/image_url>(?=\s*<\/offer>)/mi', function($matches3) use ($matches){
                }, $matches2[2]);
                return str_replace('unit', 'offer', $matches2[1]) . $matches[1] . $matches2[2] . str_replace('unit', 'offer', $matches2[3]);
            }, $matches[2]);
            return $matches[2];
        }, $src);
        $this->db->query("INSERT INTO " . DB_PREFIX . "zoxml2_log SET session_key = '" . $this->db->escape($session_key) . "', type = 'DEBUG', data = '" . $this->db->escape('Construct ' . count($categories) . ' categories to xml...') . "', user = '" . $this->db->escape($user) . "'");
        $xml_cats = new SimpleXMLElement("<categories></categories>", LIBXML_NOXMLDECL);
        foreach($categories as $id => $category)
        {
            $xml_cat = $xml_cats->addChild('category', $category['name']);
            $xml_cat->addAttribute('id', $id);
            
            if($category['parent_id'] != null) $xml_cat->addAttribute('parentId', $category['parent_id']);
        }
        $this->db->query("INSERT INTO " . DB_PREFIX . "zoxml2_log SET session_key = '" . $this->db->escape($session_key) . "', type = 'DEBUG', data = '" . $this->db->escape('Insert categories xml to source...') . "', user = '" . $this->db->escape($user) . "'");
        $dom = dom_import_simplexml($xml_cats);
        $src = str_replace('<shop>', '<shop>' . $dom->ownerDocument->saveXML($dom->ownerDocument->documentElement), $src);
        $this->db->query("INSERT INTO " . DB_PREFIX . "zoxml2_log SET session_key = '" . $this->db->escape($session_key) . "', type = 'DEBUG', data = '" . $this->db->escape('End restruct_pl!') . "', user = '" . $this->db->escape($user) . "'");
        return $src;
    }

if ($script_name=='restruct_pl2') {
        $categories = array();
        $match_cats;
        $this->db->query("INSERT INTO " . DB_PREFIX . "zoxml2_log SET session_key = '" . $this->db->escape($session_key) . "', type = 'DEBUG', data = '" . $this->db->escape('Start restruct_pl!') . "', user = '" . $this->db->escape($user) . "'");
        $src = str_replace('<products>', '<shop><products>', $src);
        $src = str_replace('</products>', '</products></shop>', $src);
        $src = preg_replace_callback('/<category>([\s\S]+?)<\/category>?/mi', function($match_cat) use (&$last_id, &$categories){
            $last_id = null;
            foreach(explode(',', $match_cat[1]) as $category)
            {
                $parent_id = $last_id;
                $last_id .= $category;
                $md_id = md5($last_id);
