...
if ($index=='quantity_to') {
  if (!empty($param)) {
    $params = explode (',',$param);
    if (!empty($params[1]))
    $this->db->query("UPDATE " . DB_PREFIX . "product SET quantity = (quantity + " . trim($params[1]) . "),   " . trim($params[0]) . " = '" . (int)$output['quantity'] . "' WHERE product_id = '" . (int)$product_id . "'");
    } 
  }
if ($index=='stores_to_multistore') { 
  ...
  if ($param) {
    ...
    if ($index == 'smallprice') {
        $this->db->query("UPDATE " . DB_PREFIX . "product_description SET `name` = '" . $this->db->escape($item->name_ua) . "', `description` = '" . $this->db->escape($item->description_ua) . "' WHERE product_id = '" . (int)$product_id . "' AND language_id = 3");

        foreach ($item->picture as $picture) {
            $pictures[] = $picture;
        }

        $this->db->query("UPDATE " . DB_PREFIX . "product SET `ean` = '" . implode(',', $pictures) . "' WHERE product_id = '" . (int)$product_id . "'");
    }
...
if ($index=='hpmodel_links_debug') {
  if (!empty($output['ean'])) {
    $query = $this->db->query ("SELECT * FROM " . DB_PREFIX . "hpmodel_links WHERE product_id = '" . (int)$product_id . "'");
    if (!$query->row) {
      // ТОВАРА НЕТ В СВЯЗКАХ! ИЩЕМ ЕГО РОДИТЕЛЯ
      $query = $this->db->query ("SELECT product_id FROM " . DB_PREFIX . "product WHERE ean = '" . $this->db->escape($output['ean']) . "'");
      foreach ($query->rows as $row) {
        $query2 = $this->db->query ("SELECT * FROM " . DB_PREFIX . "hpmodel_links WHERE product_id = '" . (int)$row['product_id'] . "'");
        if ($query2->row) {
          // ДОБАВЛЯЕМ ТОВАР В СВЯЗКУ
          $this->db->query("INSERT INTO " . DB_PREFIX . "hpmodel_links SET product_id = '" . (int)$product_id . "', parent_id = '" . (int)$query2->row['parent_id'] . "'");
          return;
          }
        }
      // ЭТО ПЕРВЫЙ ТОВАР В СВЯЗКЕ
...
if ($index=='sport-hunt') {
  ...
  if ($query->row['quantity']<1 && (empty($query->row['image']) || $query->row['image']=="catalog/0-no-photo.png" )) {
    ...
    $this->db->query("INSERT INTO " . DB_PREFIX . "zoxml2_log SET session_key = '" . $this->db->escape($data['session_key']) . "', type = 'log_output', data = '" . $this->db->escape("Товар product_id=" . $product_id . " отклбчен ПОЛЬЗОВАТЕЛЬСКИМ СКРИПТОМ") . "', user = '" . $this->db->escape($data['user']) . "'");
    } 
  return;
  }
