...
if ($index=='Explode_gorgany') {
  $items = '';
  foreach ($xml->items->item as $item) {
    if (isset($item->models->model)) foreach ($item->models->model as $model) {
      $items .= '<model>';
      $items .= '<hpmodel_link>' . trim($item['id']) . '</hpmodel_link>';
      $items .= '<name>' . trim($item->name) . '</name>';
      $items .= '<priceuah>' . trim($item->priceuah) . '</priceuah>';
      $items .= '<categoryId>' . trim($item->categoryId) . '</categoryId>';
      $items .= '<vendor>' . trim($item->vendor) . '</vendor>';
      $items .= '<description><![CDATA[' . trim($item->description) . ']]></description>';
      $items .= '<available>' . trim($item->available) . '</available>';
      $items .= '<country>' . trim($item->country) . '</country>';
      $items .= '<fcod>' . trim($model->fcod) . '</fcod>';
      $items .= '<modelname>' . trim($model->modelname) . '</modelname>';
      $items .= '<modelimage>' . trim($model->modelimage) . '</modelimage>';
      $items .= '</model>';
      }
    else {
      $items .= '<model>';
      ...
      $items .= '<available>' . trim($item->available) . '</available>';
      $items .= '<country>' . trim($item->country) . '</country>';
      $items .= '<fcod>' . trim($item->fcod) . '</fcod>';
      $items .= '<modelname>' . trim($item->name) . '</modelname>';
      $items .= '<modelimage>' . trim($item->image) . '</modelimage>';
      $items .= '</model>';
      }
    }
  $items = '<models>' . $items . '</models>';
  $new_xml = simplexml_load_string ($items, 'SimpleXMLElement', LIBXML_COMPACT | LIBXML_PARSEHUGE); 
  if ($new_xml) $data['offers'] = $new_xml;
  else {
    $dest = DIR_CACHE . $data['session_key'] . ".MODELS.YML";
    file_put_contents ($dest, $items );
    $this->db->query("INSERT INTO " . DB_PREFIX . "zoxml2_log SET session_key = '" . $this->db->escape($data['session_key']) . "', type = 'log_output', data = '" . $this->db->escape("Ошибка создания списка моделей") . "', user = '" . $this->db->escape($data['user']) . "'");
    foreach(libxml_get_errors() as $error) {
      $this->db->query("INSERT INTO " . DB_PREFIX . "zoxml2_log SET session_key = '" . $this->db->escape($data['session_key']) . "', type = 'error', data = '" . $this->db->escape('Ошибка: ' . $error->message) ."', user = '" . $this->db->escape($data['user']) . "'");
      }
    }
  return $data;
  }
  ...
  if (!empty($data['options']['accessories_group_id']['dest_id'])) {
    foreach ($xml->accessories->accessory as $item) {
      ...
      $data['dveri_com_accessories'][] = array (
        ...
        'price'         => $price,
        'price_prefix'  => "+",
        'image'         => trim($item->pictures->picture),
        'subtract'      => 0,
        'quantity'      => 1,
        'required'      => 0,
        );
