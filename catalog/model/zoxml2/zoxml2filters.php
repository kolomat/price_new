...
if ($index=='pm_height') { 
  if (isset($item->param)) {
    foreach ($item->param as $value) {
      if ($value['name']=='Высота') {
        $val = trim($value);
        if ($val>0) $output['height'] = $val; 
        }
      }
    }
  return $output;
  }
if ($index=='vesta-alpha-elite-line') { 
  $parts = explode (' ',$output['sku']);
  $output['ean'] = md5 ( $parts[0] . "::" . $session_key);
// ОПЦИИ
  if ($data['options']['param_Персонализация']['dest_id']) {
    $output['option'][] = array (
      'option_id'     => $data['options']['param_Персонализация']['dest_id'], 
      'value'         => 'Без нанесения',
      'required'      => 0,
      'price'         => 0,
      'price_prefix'  => '+',
  ...
  foreach ($item->param as $value) {
    if ($value['name']=='Персонализация' && $data['options']['param_Персонализация']['dest_id']) {
...
//
  $output['sku'] = $parts[0];
  if (!empty($parts[1])) $output['sku'] .= $parts[1];
...
if ($index=='antey_catalog_summa') { 
//<product_availability_local>false</product_availability_local>
//<product_availability>true</product_availability>
  $output['quantity'] = 0;
  if (isset($item->product_availability_local)) {
    if (trim($item->product_availability_local) == "true") $output['quantity'] += 100;
    }
  if (isset($item->product_availability)) {
    if (trim($item->product_availability) == "true") $output['quantity'] += 100;
    }
  return $output;
  }
