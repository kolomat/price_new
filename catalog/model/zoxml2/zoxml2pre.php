...
if ($data['settings']['user_pre']=='o_v_image') { 
  $output['o_v_image'] = trim($item->picture); 
  return $output;
  }
if ($data['settings']['user_pre']=='dominanta') { 
  $output['name'] = $item->group . " " . $item->format; 
  return $output;
  }
...
if ($data['settings']['user_pre']=='md5pl2ean') { 
  if (isset($item->size)) {
    $parts = explode('|', str_replace(" " . trim($item->size) . " ", '|', trim($item->name)));
