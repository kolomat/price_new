  ...
  if (isset($data['options'][$option_key])) {
    switch ($data['options'][$option_key]['dest_type']) {
      case 'option_value':              { 
        ...
        if (!empty($output['o_v_image']))       $option['o_v_image']    = $output['o_v_image'];
  ...
  if (isset($data['options'][$option_key])) {
    switch ($data['options'][$option_key]['dest_type']) {
      ...
      case 'images_over_comma': { 
        $urls = explode (',', trim((string)$value));
        foreach ($urls as $url) if (trim($url)) $output['image'][] = $data['settings']['img_path'] . trim($url);
        break; 
        } 
...
public function arrDamp2Array ($txt, $item, $prefix) {
  if (is_array($item)) {
    foreach ($item as $value) {
      ...
      else                  if ($value) $txt[] =  $prefix . (string)$value;
      }
    }
  else                      if ($value) $txt[] = $prefix . (string)$item;
  return $txt;
  }
...
public function saveOptions ($options,$product_id, $data, $relatedoptions_id=0, $summ_quantity=FALSE) {
  ...
  //  improvedoptions support
     ...
     // -------  o_v_image
      if (!empty($option['o_v_image'])) {
        $img = $this->loadOptionImage ($option['o_v_image'], $data, $option_id);
        $extra .= "o_v_image = '" . $this->db->escape($img) . "', "; 
        }
