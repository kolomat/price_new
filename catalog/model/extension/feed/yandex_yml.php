<?php
class ModelExtensionFeedYandexYml extends Model {
    ...
    public function getProduct($allowed_categories, $blacklist_type, $blacklist, $out_of_stock_ids, $allowed_manufacturers = '', $customer_group = 1, $with_related = false, $skip = 0, $limit = 200, $is_main_category = false) {
        ...
        $additional_language_id = 3; // ID дополнительного языка, например, украинского

        $query = $this->db->query("SELECT
        p.*, 
        pd.*, 
        pd_ua.name as name_ua, 
        pd_ua.description as description_ua, 
        m.name AS manufacturer, 
        p2c.category_id, 
        IFNULL(pd2.price, p.price) AS price, 
        ps.price AS special, 
        ...
        LEFT JOIN " . DB_PREFIX . "product_description pd_ua ON (p.product_id = pd_ua.product_id AND pd_ua.language_id = '" . $additional_language_id . "')
        LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id)
        ...
        LEFT JOIN " . DB_PREFIX . "weight_class_description wcd ON (p.weight_class_id = wcd.weight_class_id AND wcd.language_id='" . $language_id . "')"
